...
def build_body(data: dict, old: dict[str, dict] | None, new: dict[str, dict], today: str) -> str:
    ...
    if old is None:
        ...
        for model_id, new_m in new.items():
            if model_id not in old:
                inp = f"**{new_m['provider']}** {new_m['model_name']}"
                added.append(inp)
                continue
            old_m = old[model_id]
    ...
    # Current snapshot highlights: lowest and highest RPM per provider
    lines += ["### Current limits snapshot", ""]
    ...
    for provider, pmodels in sorted(by_provider.items()):
        ...
        lines.append(f"| {provider} | {n} | {', '.join(sorted(tiers))} |")
...
def main() -> None:
    ...
    if len(snapshots) >= 2:
        # Use the snapshot immediately before the latest
